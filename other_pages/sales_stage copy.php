<?php
    ...
    if(isset($_GET['stage_id'])){
        // echo $_GET['cu_cat_id'];
        $enquiry_stagequery = mysqli_query($conn, "SELECT * FROM `enquiry_stage`  where stage_id = " .$_GET['stage_id']);
          $enquiry_stagedata = mysqli_fetch_assoc($enquiry_stagequery);
...
<?php include '../include/sidebar.php' ?>
        ...
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            ...
            <form class="p-4" name="sales_stageform" action="../include/function.php" id="sales_stageform">
                <div>
                    ...
                    <input type="text"  name="<?php echo  isset($_GET['stage_id']) ?  'sales_stage_update' : 'sales_stage' ?>" id="sales_stage" value="<?php echo  isset($_GET['stage_id']) ? $enquiry_stagedata['stage_name'] : "" ?>" placeholder="Customer Type" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="sales_stage_update_id" value="<?php echo  isset($_GET['stage_id']) ? $_GET['stage_id'] : "" ?>" id="sales_stage_name"  class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
    ...
    <script>
        ...
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
                ...
                const Link = document.querySelector('a[href="/erp/other_pages/sales_stage.php"]');
                if (Link) {
    ...
    if(isset($_GET['stage_id'])){
        ...
        echo "<script> openModalhere('sales_stagePopup') </script>";
          }
    ?>
</body>
