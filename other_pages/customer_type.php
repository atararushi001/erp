<?php
    ...
    if(isset($_GET['cu_ty_id'])){

        $customer_type_query = mysqli_query($conn, "SELECT * FROM `customer_type`  where cu_ty_id   = " .$_GET['cu_ty_id']);
          $customer_type_data = mysqli_fetch_assoc($customer_type_query);
...
<body class="bg-gray-100" id="body">
    ...
    <div id="mydiv" class="max-w-full mx-auto mt-20 ml-16 p-4 text-sm transition-all duration-300">
        <div class="bg-white shadow-sm p-4">
            <div class="flex justify-between">
                <h1 class="text-xl font-semibold">Customer Type</h1>
                <button class="text-white text-sm px-4 py-2" id="openCCategory" style="background-color: #007bff;" onclick="openModalhere('customer_typePopup')">
                    Add Customer Type
                </button>
            </div>
    ...
    <div class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 p-4 transition-all duration-300" style="  z-index: 99;" id="customer_typePopup" style="display: BLOCK;">
        <div id="ct_popup" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            ...
            <form class="p-4" name="customer_typefrom" action="../include/function.php" id="customer_typefrom">
                <div>
                    ...
                    <input type="text"name="<?php echo  isset($_GET['cu_ty_id']) ?  'customer_type_update' : 'customer_type' ?>"  value="<?php echo  isset($_GET['cu_ty_id']) ? $customer_type_data['cu_ty_name'] : "" ?>"  id="customer_type" placeholder="Customer Type" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="customer_type_update_id" value="<?php echo  isset($_GET['cu_ty_id']) ? $_GET['cu_ty_id'] : "" ?>" id="customer_type_update_id" >
                
                </div>
                <div class="flex items-center justify-start gap-4 mt-32">
                    <button class="text-white text-sm px-4 py-2 w-28" onclick="savedatanow('customer_typefrom',<?php echo  isset($_GET['cu_ty_id']) ?  'customer_type_update' : 'customer_type_update_id' ?>)" type="button" id="openButton" style="background-color: #007bff;"><?php echo  isset($_GET['cu_ty_id']) ?  'Edit' : 'Save' ?></button>
    ...
    <script>
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
              ...
              const Link = document.querySelector('a[href="/erp/other_pages/customer_type.php"]');
              if (Link) {
        ...
        function savedatanow(formname,inputselecter) {
            ...
            $.ajax({
                ...
                success: function(data) {
                 ...
                 window.location.href = "customer_type.php";
                },
                error: function(data) {
                    alert("some Error");
                }
    ...
    if(isset($_GET['cu_ty_id'])){
        ...
        echo "<script> openModalhere('customer_typePopup') </script>";
          }
    ?>
</body>
