<?php
    ...
    if(isset($_GET['product_category_id'])){
        // echo $_GET['cu_cat_id'];
        $product_categoryquery = mysqli_query($conn, "SELECT * FROM `product_category`  where product_category_id = " .$_GET['product_category_id']);
          $product_categorydata = mysqli_fetch_assoc($product_categoryquery);
...
<?php include '../include/sidebar.php' ?>
    ...
    <div class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 p-4 transition-all duration-300" style="  z-index: 99;" id="product_categoryPopup" style="display: BLOCK;">
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            <div class="flex justify-between border-b">
                ...
                <svg id="closeCCategory" onclick="closeModal('product_categoryPopup')" class="cursor-pointer mt-3 mr-2 close-button" width="35" height="35" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <form class="p-4" name="product_categoryform" action="../include/function.php" id="product_categoryform">
                <div>
                    <label for="customer_type" class="text-gray-700 font-semibold">product_category</label>
                    <input type="text"  name="<?php echo  isset($_GET['product_category_id']) ?  'product_category_update' : 'product_category' ?>" id="product_category" value="<?php echo  isset($_GET['product_category_id']) ? $product_categorydata['product_category_name'] : "" ?>" placeholder="Product Category" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="product_category_update_id" value="<?php echo  isset($_GET['product_category_id']) ? $_GET['product_category_id'] : "" ?>" id="getproduct_category_name"  class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                
                </div>
                <div class="flex items-center justify-start gap-4 mt-32">
                    <button class="text-white text-sm px-4 py-2 w-28" 
                    onclick="savedatanow('product_categoryform','<?php echo isset($_GET['product_category_id']) ? 'product_category_update' : 'product_category' ?>')"
                   type="button" id="openButton" style="background-color: #007bff;">Save</button>
                    <button type="button" class="border bg-white text-sm px-4 py-2 w-28" onclick="closeModal('product_categoryPopup')" style="color: #007bff; border: 1px solid #007bff;">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    if(isset($_GET['product_category_id'])){
        ...
        echo "<script> openModalhere('product_categoryPopup') </script>";
          }
    ?>
</body>
