<?php
    ...
    if(isset($_GET['job_title_id'])){
        $job_title_query = mysqli_query($conn, "SELECT * FROM `job_title`  where job_title_id   = " .$_GET['job_title_id']);
          $job_title_data = mysqli_fetch_assoc($job_title_query);
    ...
    <div id="mydiv" class="max-w-full mx-auto mt-20 ml-16 p-4 text-sm transition-all duration-300">
        <div class="bg-white shadow-sm p-4">
            <div class="flex justify-between">
                ...
                <button class="text-white text-sm px-4 py-2" id="openCCategory" style="background-color: #007bff;" onclick="openModalhere('job_titledataPopup')">
                    Add Job Title
                </button>
            </div>
        ...
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            ...
            <form class="p-4" name="job_titlefrom" action="../include/function.php" id="job_titlefrom">
                <div>
                    ...
                    <input type="text" name="<?php echo isset($_GET['job_title_id']) ? 'job_title_name_update' : 'job_titledata' ?>" value="<?php echo isset($_GET['job_title_id']) ? $job_title_data['job_title_name'] : "" ?>" name="job_titledata" id="job_titledata" placeholder="job_title" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="job_title_id_update" value="<?php echo isset($_GET['job_title_id']) ? $_GET['job_title_id'] : "" ?>" id="job_title_id" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
               
                </div>
                <div class="flex items-center justify-start gap-4 mt-32">
                    <button class="text-white text-sm px-4 py-2 w-28"onclick="savedatanow('job_titlefrom', '<?php echo isset($_GET['job_title_id']) ? 'job_title_name_update' : 'job_titledata' ?>')" 
    ...
    <script>
        ...
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
                ...
                const Link = document.querySelector('a[href="/erp/other_pages/job_title.php"]');
                if (Link) {
...
<?php 
    if(isset($_GET['job_title_id'])){
        ...
        echo "<script> openModalhere('job_titledataPopup') </script>";
          }
    ?>
</html>