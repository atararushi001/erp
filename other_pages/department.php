<?php
    ...
    if(isset($_GET['department_id'])){
        $department_query = mysqli_query($conn, "SELECT * FROM `department`  where department_id   = " .$_GET['department_id']);
...
<?php include '../include/sidebar.php' ?>
    ...
    <div id="mydiv" class="max-w-full mx-auto mt-20 ml-16 p-4 text-sm transition-all duration-300">
        <div class="bg-white shadow-sm p-4">
            <div class="flex justify-between">
                <h1 class="text-xl font-semibold">Department</h1>
                <button class="text-white text-sm px-4 py-2" style="background-color: #007bff;" onclick="openModalhere('departmentdataPopup')">
                    Add Department
                </button>
            </div>
    ...
    <div class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 p-4 transition-all duration-300" style="    z-index: 99;" id="departmentdataPopup" style="display: BLOCK;">
        <div id="department_popup" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            ...
            <form class="p-4" name="departmentdatafrom" action="../include/function.php" id="departmentdatafrom">
                <div>
                    ...
                    <input type="text" name="<?php echo isset($_GET['department_id']) ? 'department_id_name_update' : 'departmentdata' ?>" value="<?php echo isset($_GET['department_id']) ? $department_data['department_name'] : "" ?>" name="departmentdata" id="departmentdata" placeholder="Department" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="department_id_update" value="<?php echo isset($_GET['department_id']) ? $_GET['department_id'] : "" ?>" id="department_id" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
               
                </div>
                <div class="flex items-center justify-start gap-4 mt-32">
                    <button class="text-white text-sm px-4 py-2 w-28"onclick="savedatanow('departmentdatafrom', '<?php echo isset($_GET['department_id']) ? 'department_id_name_update' : 'departmentdata' ?>')" 
    ...
    <script>
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
                ...
                const Link = document.querySelector('a[href="/erp/other_pages/department.php"]');
                if (Link) {
...
<?php 
    if(isset($_GET['department_id'])){
        ...
        echo "<script> openModalhere('departmentdataPopup') </script>";
          }
    ?>
</html>