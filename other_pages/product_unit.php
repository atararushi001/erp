...
<body class="bg-gray-100" id="body">
    ...
    <div id="mydiv" class="max-w-full mx-auto mt-20 ml-16 p-4 text-sm transition-all duration-300">
        <div class="bg-white shadow-sm p-4">
            <div class="flex justify-between">
                <h1 class="text-xl font-semibold">Master Product Unit List</h1>
                <button class="text-white text-sm px-4 py-2" id="openProductUnit" style="background-color: #007bff;">
                    Add Unit
                </button>
            </div>
    ...
    <div class="fixed inset-0 items-center justify-center bg-black bg-opacity-50 p-4 transition-all duration-300"
        id="productUnitPopup">
        <div id="pu_popup" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg"
            style="width: 600px;">
            <div class="flex justify-between border-b">
                <h2 class="text-gray-800 font-semibold p-4 text-xl">Create new Unit</h2>
                <svg id="closeProductUnit" onclick="closeProductUnit()" class="cursor-pointer mt-3 mr-2 close-button"
            ...
            <form class="p-4">
                <div>
                    <label for="product_unit_name" class="text-gray-700 font-semibold">Unit Name</label>
                    <input type="text" id="product_unit_name" placeholder="Unit Name"
    ...
    <script>
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
              ...
              const Link = document.querySelector('a[href="/erp/other_pages/product_unit.php"]');
              if (Link) {
        ...
        const productUnitPopup = document.getElementById("productUnitPopup");
        const openProductUnit = document.getElementById("openProductUnit");
        const closeProductUnit = document.getElementById("closeProductUnit");

        openProductUnit.addEventListener("click", () => {
            productUnitPopup.style.display = "block";
            body.classList.remove("hidden");
        });

        closeProductUnit.addEventListener("click", () => {
            productUnitPopup.style.display = "none";
        });
    </script>
