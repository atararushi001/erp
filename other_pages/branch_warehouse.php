<?php
    ...
    if(isset($_GET['warehouse_id'])){
        ...
        $customer_warehousequery = mysqli_query($conn, "SELECT * FROM `warehouse`  where warehouse_id   = " .$_GET['warehouse_id']);
          $customer_warehousedata = mysqli_fetch_assoc($customer_warehousequery);
...
<?php include '../include/sidebar.php' ?>
    ...
    <div id="mydiv" class="max-w-full mx-auto mt-20 ml-16 p-4 text-sm transition-all duration-300">
        <div class="bg-white shadow-sm p-4">
            <div class="flex justify-between">
                <h1 class="text-xl font-semibold">Branch/Warehouse</h1>
                <button class="text-white text-sm px-4 py-2" id="openCCategory" style="background-color: #007bff;" onclick="openModalhere('customer_branch_warehousePopup')">
            ...
            <table id="customerTable" class="min-w-full table-auto">
              ...
              <?php getcustomerbranch_warehouse();  ?>
        ...
        <div id="bw_popup" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-sm shadow-lg" style="width: 600px;">
            ...
            <form class="p-4" name="customer_branch_warehouseform" action="../include/function.php" id="customer_branch_warehouseform">
                <div>
                    ...
                    <input type="text" name="<?php echo  isset($_GET['warehouse_id']) ?  'customer_branch_warehouse_update' : 'customer_branch_warehouse' ?>"  value="<?php echo  isset($_GET['warehouse_id']) ? $customer_warehousedata['warehouse_name'] : "" ?>" id="customer_branch_warehouse" placeholder="Customer Type" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
                    <input type="hidden" name="customer_branch_warehouse_update_id" value="<?php echo  isset($_GET['warehouse_id']) ? $_GET['warehouse_id'] : "" ?>" placeholder="Customer Category Name" class="border rounded-sm outline-none p-2 w-full focus:ring focus:ring-blue-400 mt-2">
              
                </div>
                <div class="flex items-center justify-start gap-4 mt-32">
                    <button class="text-white text-sm px-4 py-2 w-28" onclick="savedatanow('customer_branch_warehouseform',<?php echo  isset($_GET['warehouse_id']) ?  'customer_branch_warehouse_update' : 'customer_branch_warehouse' ?>)" type="button" id="openButton" style="background-color: #007bff;">Save</button>
    ...
    <script>
        $(document).ready(function() {
            if(window.location.href.includes("other_pages")) {
              ...
              const Link = document.querySelector('a[href="/erp/other_pages/branch_warehouse.php"]');
              if (Link) {
    ...
    function savedatanow(formname,inputselecter) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    window.location.href = "branch_warehouse.php";
    ...
    if(isset($_GET['warehouse_id'])){
        ...
        echo "<script> openModalhere('customer_branch_warehousePopup') </script>";
          }
    ?>
</body>
